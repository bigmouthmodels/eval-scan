...
def uniform(df: pd.DataFrame, con: duckdb.DuckDBPyConnection):
    ...
    async def dummy_scorer(row: pd.Series):
        ...
        metadata = ""
        con.execute(
            "INSERT INTO message_scores VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            [
                uuid,
                message_uuid,
                inserted,
    ...
    existing_scores = con.execute(
        "SELECT * FROM message_scores WHERE score_name = (?)", [score_name]
    ).df()
    apply_async(
        df[~df["uuid"].isin(existing_scores["message_uuid"].unique())], dummy_scorer
    )
